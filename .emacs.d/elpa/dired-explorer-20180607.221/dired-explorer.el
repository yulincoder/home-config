...
;;
;; [ja]
;; WindowsやMac OS Xのデフォルトのファイラのようなファイル联买をdiredで乖います。
;; 毖眶机のキ〖を虑赴するだけで、diredでファイル】ディレクトリを联买します。
;; また、diredeがたくさんのバッファを倡きすぎることを娃贿しています。
;; 碰脸ながら、このモ〖ドを铜跟にするとデフォルトのdiredのキ〖バインドが蝗えません。
;; diredのアルファベット办矢机のキ〖バインドは答塑弄に"M-"にあて木しています。
;; モ〖ドの磊り仑えは":"で乖ってください。
;; このelispは、るびきちさんが揉のEmacs轿で、送にURLを兜えてくれましたが、
;; 送にはオリジナルの侯荚が茂かわからなかったので、URLだけ汤绩しています。
